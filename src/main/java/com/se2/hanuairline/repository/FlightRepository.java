...
//    List<Flight> findFlightByAirway_Id(Long id);
    ...
    @Query(value ="SELECT * from flight WHERE "
    		+ "(arrival_time BETWEEN ?1 - INTERVAL 30 MINUTE and ?1 + INTERVAL 30 MINUTE AND arrival_gate_id = ?3 )"
    		+ "OR (departure_time BETWEEN ?2 - INTERVAL 30 MINUTE and ?2 + INTERVAL 30 MINUTE AND departure_gate_id = ?4)", nativeQuery = true)
